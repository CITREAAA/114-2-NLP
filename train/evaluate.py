...
TOKENIZER_NAME = "cardiffnlp/twitter-roberta-base-sentiment"  # ← 固定用原始 tokenizer
...
parts = [
    grp.sample(min(len(grp), sample_per_class), random_state=RANDOM_SEED)
...
# ================= 4. 載入模型與 Tokenizer =================
print(f"\n正在載入 Tokenizer：{TOKENIZER_NAME}")
print(f"正在載入模型權重：{MODEL_PATH}")
tokenizer = AutoTokenizer.from_pretrained(TOKENIZER_NAME)   # ← 關鍵修正
...
wrong = sampled[~sampled['is_correct']].copy()
